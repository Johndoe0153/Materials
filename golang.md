...
또한 선언된 변수가 사용되지 않으면 에러가 발생한다. 그리고 동일한 타입의 변수가 여러 개 있다면, 변수들을 나열하고 마지막에 타입을 한 번만 지정한다.
...
>**포인터**: 변수 파트에서 **"변수는 특정 함수 밖에 있는 경우에는 다른 함수에서도 접근 가능하지만, 함수 안에 있다면 다른 함수에서 접근할 수 없다."**
...
`포인터`를 사용하면 함수에 `msg` 변수의 값을 복사하지 않아도 `msg` 변수의 주소를 전달하게 된다. 그렇게 되면 피호출 함수 `say()`는 `*string`과 같이 매개변수가 `포인터`임을 표시하고 이때 `say` 함수의 `msg`는 문자열이 아니라 문자열이 갖는 메모리 영역의 주소를 갖게 된다. 그래서 `msg` 주소에 데이터를 쓰기 위해서는 `*msg = "어쩌고"`와 같이 앞에 `*`를 붙이는 데 이를 `역참조`라고 부른다.

>**역참조**: 포인터가 가리키는 값을 가져오는 것

또한 함수에 고정된 갯수의 매개변수들을 전달하지 않고 여러 개의 매개변수를 전달할 떄 가변 매개변수를 나타내기 위해 `...(3개의 마침표)`를 사용한다. 그리고 이런 가변 매개변수를 갖는 함수를 호출할 때는 n개의 동일한 타입의 매개변수를 전달할 수 있다. 이러한 함수를 `가변인자함수`라고 부른다.
...
`say` 함수에 4개의 문자열을 전달할 수도, 1개의 문자열을 전달할 수 도 있음을 보여준다.

Go 에서 함수의 리턴값은 없을 수도, 하나일 수도, 또는 여러 개 일수 있다.

>**리턴**: 반환 및 종료를 의미한다. `return` 문을 만나면 현재 함수를 종료하고 호출한 곳으로 이동한다. 만일 반환 값이 있다면 해당 값과 함께 호출한 곳으로 이동하게 된다. 만일 `main()` 함수에서 `return` 문을 만나면 해당 프로그램을 종료한다.

 함수에서 리턴값이 있는 경우는 `func` 문의 마지막에 리턴 값의 타입을 정의해 준다. 그리고 값을 리턴하기 위해서 함수 내에서 `return` 키워드를 사용하게 된다. 아래 코드에서는
...
`sum()` 함수의 리턴 타입이 `int` 임을 표시하고 있으며, `sum` 함수 마지막에 `return s` 와 같이 정수 `s` 의 값을 리턴하고 있다.

Go 에서 여러 개의 값을 리턴하려면 해당 리턴 타입들을 `괄호()` 안에 적어주면 된다. 예를 들어 처음과 둘째 리턴 값이 각각 `int` 과 `string` 이면 `(int, string)` 처럼 적는다.

또한 Go 는 `Named Return Parameter` 라는 기능으로 함수에서 정의된 리턴되는 값들을 리턴 매개변수에 할당할 수 있다.